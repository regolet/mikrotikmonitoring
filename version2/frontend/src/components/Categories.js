...
function Categories() {
  ...
  const handleShowEditSubcategory = (catIdx, subIdx) => {
    ...
    setSubcategoryName(categories[catIdx].subcategories[subIdx]?.subcategory || '');
    setSubcategoryError('');
    setShowSubcategoryModal(true);
  };
  ...
  const handleShowAssignGroups = async (catIdx, subIdx) => {
    ...
    try {
      const res = await axios.get(`${API_BASE_URL}/groups?router_id=${activeRouterId}`);
  ...
  if (!categories.length) {
    return (
      <div className="container-fluid mt-4">
        <div className="alert alert-info">No categories found for this router.</div>
      </div>
    );
  }
    ...
    <div className="container-fluid mt-4">
      ...
      <div className="categories-list">
        {categories.map((cat, catIdx) => {
          ...
          const { totalMembers: catTotalMembers, totalMax: catTotalMax, hasAllMax: catHasAllMax } = getMembersStats(catGroupIds);
          let catCountStr = '';
          ...
          return (
            <div className="category-card card mb-3" key={catIdx}>
              <div className="card-header d-flex justify-content-between align-items-center fw-bold">
                <span>{cat.category} {catCountStr && <span className="badge bg-primary ms-2">{catCountStr}</span>}</span>
                <span>
                  <button className="btn btn-sm btn-outline-success me-1" style={{background:'transparent'}} title="Add Subcategory" onClick={() => handleShowAddSubcategory(catIdx)}>
                    <i className="bi bi-plus"></i>
                  </button>
                  <button className="btn btn-sm btn-outline-secondary me-1" style={{background:'transparent'}} title="Edit Category" onClick={() => handleShowEditCategory(catIdx)}>
                    <i className="bi bi-pencil"></i>
                  </button>
                  <button className="btn btn-sm btn-outline-danger" style={{background:'transparent'}} title="Delete Category" onClick={() => handleDeleteCategory(catIdx)}>
              ...
              {Array.isArray(cat.subcategories) && cat.subcategories.length > 0 ? (
                <div className="row g-2 px-2 pb-2">
                  {cat.subcategories.map((sub, subIdx) => {
                    const { totalMembers: subTotalMembers, totalMax: subTotalMax, hasAllMax } = getMembersStats(sub.groups || []);
                    let subcatCountStr = '';
                      ...
                      <div className="col-12 col-md-6 col-lg-4" key={subIdx}>
                        <div className="card h-100">
                          <div className="card-header d-flex justify-content-between align-items-center">
                            <span>
                              {sub.subcategory} {subcatCountStr && <span className="badge bg-primary ms-2">{subcatCountStr}</span>}
                            </span>
                            <span>
                              <button className="btn btn-sm btn-outline-primary me-1" style={{background:'transparent'}} title="Assign Groups" onClick={() => handleShowAssignGroups(catIdx, subIdx)}>
                                <i className="bi bi-collection"></i>
                              </button>
                              <button className="btn btn-sm btn-outline-secondary me-1" style={{background:'transparent'}} title="Edit Subcategory" onClick={() => handleShowEditSubcategory(catIdx, subIdx)}>
                                <i className="bi bi-pencil"></i>
                              </button>
                              <button className="btn btn-sm btn-outline-danger" style={{background:'transparent'}} title="Delete Subcategory" onClick={() => handleDeleteSubcategory(catIdx, subIdx)}>
                                <i className="bi bi-trash"></i>
                              </button>
                            </span>
                          </div>
                          <div className="card-body">
                            {Array.isArray(sub.groups) && sub.groups.length > 0 ? (
                              <div>
                                {sub.groups.map((gid, gidx) => {
                                  ...
                                  if (g) {
                                    ...
                                    return (
                                      <div key={gidx} className="mb-1">
                                        <span className="badge bg-info text-dark">
                                          {g.name} ({total}{typeof g.max_members === 'number' ? `/${g.max_members}` : ''})
                                        </span>
                                      </div>
                                    );
                                  } else {
                                    return (
                                      <div key={gidx} className="mb-1">
                                        <span className="badge bg-info text-dark">{gid}</span>
                                      </div>
                                    );
                                  }
                                })}
                              ...
                              <span className="text-muted"><em>No groups assigned</em></span>
                            )}
                          </div>
                        </div>
                      </div>
      ...
      {/* Category Modal */}
      <>
        <div className={modalShowClass(showCategoryModal)} tabIndex="-1" style={{ background: showCategoryModal ? 'rgba(0,0,0,0.3)' : undefined }}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{categoryModalMode === 'add' ? 'Add New Category' : 'Edit Category'}</h5>
                <button type="button" className="btn-close" onClick={() => setShowCategoryModal(false)}></button>
              </div>
              <form onSubmit={e => { e.preventDefault(); handleCategoryModalSave(); }}>
      ...
      {/* Subcategory Modal */}
      <>
        <div className={modalShowClass(showSubcategoryModal)} tabIndex="-1" style={{ background: showSubcategoryModal ? 'rgba(0,0,0,0.3)' : undefined }}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{subcategoryModalMode === 'add' ? 'Add New Subcategory' : 'Edit Subcategory'}</h5>
                <button type="button" className="btn-close" onClick={() => setShowSubcategoryModal(false)}></button>
              </div>
              <form onSubmit={e => { e.preventDefault(); handleSubcategoryModalSave(); }}>
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    />
                    {subcategoryError && <div className="invalid-feedback">{subcategoryError}</div>}
                  </div>
                </div>
                <div className="modal-footer">
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button type="button" className="btn-close" onClick={() => setShowAssignGroupsModal(false)}></button>
              </div>
              <form onSubmit={e => { e.preventDefault(); handleAssignGroupsSave(); }}>
                <div className="modal-body">
                  <div className="mb-3">
